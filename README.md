...
### plan-runner  ![version](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fraw.githubusercontent.com%2FMisterVitoPro%2Fplan-runner%2Fv2.2.0%2F.claude-plugin%2Fplugin.json&query=%24.version&label=version&prefix=v&color=blue)

**Take a Markdown implementation plan, run it as a verified task DAG of parallel agents.**

**Claude Code and Codex ready. Requires Git.** Since 3.0.0 the dependency-ready task DAG is the only executor: the wave executor and its no-Git fallback are gone, and a repository without Git, a first commit, or usable worktrees stops at pre-flight with preparation steps. Bundled analyzer, test-author, developer, verifier, integrator, and aggregator roles are delivered to subagents by file path relative to the active skill; Codex uses native subagents and Claude Code can additionally opt into Agent Teams.

The analyzer turns your plan into a task graph with stable IDs and explicit dependency edges (six concurrent tasks max, never two active tasks on the same file). Each ready task runs in its own disposable Git worktree rooted at the integration commit that satisfied its dependencies, so a fast task's children start while an unrelated slow task is still running; untracked dependencies such as `node_modules` are linked into each worktree (`worktree.share` / `worktree.setup`) instead of being rebuilt per task. The scheduler commits each task's work, checks the complete diff against declared ownership, runs its gates inside that worktree, and requests an independent verdict. Integration onto the run-owned branch is central, serial, and severity-gated: a P0 or P1 finding, an ownership failure, or a failed check earns exactly one evidence-backed repair attempt and then blocks the task and its dependents, while P2 and P3 findings ride along to the fix-plan. Gates are file-backed, time-budgeted, and never waited on in the background; roles, the plan, and test output reach agents as file paths rather than prompt text; and the full suite runs only for the baseline, at phase boundaries, and as the final verification that gates delivery. `run-state.json` plus an append-only `events.jsonl` make every dispatch, retry, block, and integration reconstructable, so every run is resumable and `--resume` never redispatches an integrated task; long runs checkpoint by integration count (`phasing.max_integrations_per_phase`) with relay/stop boundaries. The aggregator dedupes bugs (P0-P3) and emits a `fix-plan.md` whose re-run builds on the previous cycle's branch. TDD red-green mode is on by default (`--no-tdd` to disable). Your active branch and checkout are never touched, and the run ends by pushing the integration branch and opening or updating a structured pull request for human review (a draft when bugs or blocked tasks remain) -- it never auto-merges. Every subagent's token usage is tallied best-effort into `manifest.json`, with explicit lower-bound coverage when the host exposes incomplete usage data.
