...
| `plan-runner` | 2.0.1 | Dual-client Claude Code and Codex plan executor with portable bundled-role loading. Default task-DAG executor in Git repositories: dependency-ready scheduling of stable task IDs (six-task ceiling, no overlapping owned files), one disposable worktree per task, ownership-conformance diff plus independent verification before a central integrator serially applies each commit to a run-owned integration branch, one evidence-backed repair and one conflict rebuild per task, durable run-state and append-only events for resume, and integration-count phasing with relay/stop boundaries. `execution.mode: wave` is the explicit rollback and the automatic no-Git path. TDD gates, honest token/coverage accounting, bug-driven fix-plan re-runs, optional Code Atlas sync, and human-reviewed structured pull requests (never auto-merged). Codex uses native subagents; Claude Code can additionally opt into Agent Teams. Sourced externally from [github.com/MisterVitoPro/plan-runner](https://github.com/MisterVitoPro/plan-runner). |
