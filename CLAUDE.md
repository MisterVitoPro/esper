...
|--------|---------|-------------|
| `qa-swarm` | 1.6.0 | Dual-client Claude Code and Codex quality pipeline with portable bundled-role loading: parallel specialist review (Opus for security, architecture, and backwards-compat by default; selectable per run with `--model opus|sonnet|haiku`, mapped to Terra/Luna on Codex), evidence-backed P0-P3 findings, a generated implementation spec and test plan, and a fresh-context test-first implementation handoff. Sourced externally from [github.com/MisterVitoPro/qa-swarm](https://github.com/MisterVitoPro/qa-swarm). |
| `code-atlas` | 2.3.2 | Dual-client Claude Code and Codex architecture index generator with semantic graph -- writes .code-atlas/atlas.json, state.json, and graph-schema.json, loaded by a shared session-start hook. Deterministic graph queries and validation use the bundled scripts/query.js. Directory map, tech stack, patterns, dependencies. Sourced externally from [github.com/MisterVitoPro/code-atlas](https://github.com/MisterVitoPro/code-atlas). |
| `plan-runner` | 2.2.0 | Dual-client Claude Code and Codex plan executor with bundled roles delivered by file path. The task DAG is the only executor (since 3.0.0; Git with usable worktrees required, no wave executor, no no-Git fallback): dependency-ready scheduling of stable task IDs (six-task ceiling, no overlapping owned files), one disposable worktree per task bootstrapped with linked untracked dependencies, scheduler-made task commits, a mechanical ownership-conformance diff, gates run inside the task worktree under file-backed time-budgeted gate discipline, and an independent verdict before central, serial, severity-gated integration onto a run-owned branch (P0/P1 block, P2/P3 ride along; one shared repair/re-execution attempt per task). Durable run-state and append-only events make every run resumable, with integration-count phasing and relay/stop boundaries. TDD gates, a full suite only at baseline, phase boundaries, and final verification, honest token/coverage accounting, bug-driven fix-plan re-runs based on the previous cycle's branch, and human-reviewed structured pull requests (draft when bugs or blocked tasks remain; never auto-merged). Codex uses native subagents; Claude Code can additionally opt into Agent Teams. Sourced externally from [github.com/MisterVitoPro/plan-runner](https://github.com/MisterVitoPro/plan-runner). |
...
claude plugin install <plugin-name>@esper
